...
- 菜单第 3 项：骰子
- 主菜单和番茄钟菜单统一为竖屏垂直列表
- 左上角用 Wi-Fi 图标显示联网/离线状态，右上角显示电池
...
- 开始后屏幕切到竖屏沙漏
- 沙漏外形是最简六条线轮廓，顶部和底部各保留一条横线，中间不画圆圈，并尽量占满竖屏
- 蓝色颗粒按所选总时长计算流速，逐步从上半部分流到下半部分
- 沙漏运行时用 `M5Canvas` 离屏绘制后一次性推送到屏幕，减少擦黑重画导致的闪烁
- 进入竖屏沙漏后重新采集 IMU 重力基准，用于后续反转检测
- 沙漏使用颗粒守恒模型：上部颗粒数 + 下落中颗粒数 + 底部颗粒数保持为固定总量
- 底部颗粒按重力和倾斜方向下落/滑动
- 蓝色颗粒会先按沙漏轮廓裁剪，尽量限制在玻璃边界内
- 颗粒从颈部落下时就受倾斜影响，落点、滴流、顶部表面和底部堆积都会随相对重力方向变化
