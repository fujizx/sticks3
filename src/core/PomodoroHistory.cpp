...
void PomodoroHistory::add(const PomodoroRecord &record) {
...
uint8_t PomodoroHistory::count() const {
  return count_;
}

bool PomodoroHistory::get(uint8_t index, PomodoroRecord &record) const {
  if (index >= count_) return false;
  record = records_[index];
  return true;
}

void PomodoroHistory::clear() {
  count_ = 0;
  memset(records_, 0, sizeof(records_));
  persist();
}

void PomodoroHistory::persist() {
